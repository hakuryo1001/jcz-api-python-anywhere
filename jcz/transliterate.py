...
def extract_components(input, orthography="honzi_jcz"):
    ...
    targets = [
        pc.characters_to_jyutping(glyph)[0]
        for glyph in input
...
syllabics = {"ng": "五`", "m": "五."}
jping_rgx = re.compile(
    "^(([bBpPmMfFdDtTnNlLzZcCsSjJgGkKhHwrRvV]|ng|gw|kw|)((aa(i|u|m|ng|n|p|t|k|))|(a(a|i|u|m|ng|n|p|t|k|))|(oe(ng|k|))|(eo(i|n|t))|(e(i|u|m|ng|n|p|t|k|))|(i(u|m|ng|n|p|t|k|))|(o(i|u|ng|n|t|k|))|(yu(n|t|))|(u(i|ng|n|t|k|))|z)|(m|ng|er))([1-6]?)$"
)
...
def words_to_jyutping(glyphs, cur_cmu, outs, unparsed, prev_was_eng_word):
    ...
    if is_jyutping or (
        len(glyphs) == 1 and glyphs.isupper()
    ):  # re.match(jping_rgx, glyphs) or glyphs in hngs:
            ...
            for word in words:
                ...
                if parsed is not None:
                    ...
                    warnings.warn(
                        ...
                        + " from "
                        + frag
                        + " is not converted into Jyutping"
                    )
                    failed = True
                    break
            if failed:
                ...
                if re.search("[a-zA-Z]", glyphs):
                    ...
                    if orthography == "jcz_only":
                        ...
                        for i, glyph in enumerate(glyphs):
                            outs.append(
                                str(jping_arr[i]) if glyph in mouth_list else glyph
                            )
                prev_was_eng_word = False
            elif len(glyphs) == 1 and not is_alphanum(
                glyphs
            ):  # glyphs in puncs or glyphs.isdigit():
                outs.append(glyphs)
                prev_was_eng_word = False
            else:
                outs, unparsed, prev_was_eng_word = words_to_jyutping(
                    glyphs, cur_cmu, outs, unparsed, prev_was_eng_word and sep_eng_words
                )
...
def assemble(jping_tup, mode="font"):
    ...
    if is_syllabic:
        ...
        if onset == "" and final != "" and len(suffix) == 1 and not is_syllabic:
            ...
            has_zero = (onset == "" and final != "") or (
                onset != "" and final == "" and len(onset) == 1
            )
            thing = (
                (
                    onsets[onset]
                    if onset in onsets
                    else "".join(onsets[c] for c in onset)
                )
                + finals[final]
                + ("`" if has_zero else "")
                + tones[tone]
        ...
        if mode == "web":
            return (
                thing
                + ("·" if tone in {"", None} else "")
                + (suffix_thing + "·" if suffix != "" else "")
            )
        elif mode == "font":
            return hex_dict[thing] + (
                hex_dict[suffix_thing] if suffix_thing != "" else ""
            )
...
is_loweralphanum = (
    lambda x: x.isascii()
    and x.isalnum()
    and (x.islower() or (x.isdigit() and len(x) == 1))
)
...
def transliterate(
    ...
    use_repeat_char=True,
    initial_r_block="r",
    v_block="v",
    tone_config="horizontal",
    use_schwa_char=False,
    algorithm="PyCantonese",
    sep_eng_words=True,
):
    ...
    converter = (
        tojyutping_converter if algorithm == "ToJyutping" else pycantonese_converter
    )
    outs, unparsed = converter(
        input, cur_cmu, orthography=orthography, sep_eng_words=sep_eng_words
    )
    ...
    if use_repeat_char:
        if len(outs) > 1:
            ...
            while i < len(outs):
                if (
                    outs[i] == outs[i - 1]
                    and not (is_alphanum(outs[i]))
                    and not outs[i] in puncs
                ):
                    new_outs.append("々")
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--mode",
        dest="mode",
        help="use font or web-style characters",
        metavar="MODE",
        default="font",
    )
    parser.add_argument(
        ...
        "--style",
        dest="orthography",
        help="use jcz-only (jcz_only) or sinoglyph-jcz (honzi_jcz) writing style",
        metavar="STYLE",
        default="honzi_jcz",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--use_repeat_char",
        dest="use_repeat_char",
        help="whether to use the repeat glyph 々",
        metavar="BOOL",
        type=bool,
        default=True,
    )
    parser.add_argument(
        "--use_schwa_char",
        dest="use_schwa_char",
        help="whether to use 亇 for the final 'a' instead of 乍",
        metavar="BOOL",
        type=bool,
        default=False,
    )
    ...
    parser.add_argument(
        ...
        "--sep_eng_words",
        dest="sep_eng_words",
        help="whether to add whitespace between English words in the output",
        metavar="BOOL",
        type=bool,
        default=True,
    )
    ...
    if not sys.stdin.isatty():
        ...
        while True:
            ...
            if len(t) > 0:
                ...
                print(
                    "JCZ transliterator was unable to transliterate your text into JCZ."
                )
